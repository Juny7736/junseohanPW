...
const ProjectCard = ({ project, index }: ProjectCardProps) => {
  ...
  return (
    <motion.div variants={cardVariants} className="project-card card group">
      ...
      <div className="p-6">
        ...
        <div className="flex flex-wrap gap-2">
          {project.technologies.map((tech) => (
            <span
              key={tech}
              className="px-2 py-1 text-xs font-medium bg-primary-50 dark:bg-primary-700 text-primary-600 dark:text-primary-300 rounded"
            >
              {tech}
            </span>
          ))}
